...
# load data
data    = np.load('data/figs/6_' + '_'.join([str(param) for param in [N, kappa, gamma, delta_2, g_1, g_2, Omega, dim_1, dim_2]]) + '.npz')
X       = data['arr_0']
Y       = data['arr_1']
g_2_0s  = data['arr_2']
...
plotter = MPLPlotter(axes={
    ...
    'tick_font_size': 20,
    'annotations': [{
        's': '({:0.2f}, {:0.3f})'.format(pos_1[0], pos_1[1]),
        'xy': (0.428, 0.325),
        # 'xy': (0.46, 0.3), # alt
        'font_dict': 'tick',
        'color': 'k'
    }, {
        's': '{:0.2f}'.format(pos_1[2]),
        'xy': (0.56, 0.435),
        # 'xy': (0.59, 0.395), # alt
        'font_dict': 'label',
        'color': 'k'
    }, {
        's': '({:0.2f}, {:0.3f})'.format(pos_2[0], pos_2[1], pos_2[2]),
        'xy': (0.33, 0.73),
        # 'xy': (0.3, 0.755), # alt
        'font_dict': 'tick',
        'color': 'k'
    }, {
        's': '{:0.2f}'.format(pos_2[2]),
        'xy': (0.33, 0.62),
        # 'xy': (0.3, 0.65), # alt
        'font_dict': 'label',
        'color': 'k'
...
plotter.update(vs=np.log10(g_2_0s))
# additional lines
